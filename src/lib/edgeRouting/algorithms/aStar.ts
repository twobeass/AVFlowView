/**
 * A* pathfinding algorithm for edge routing
 * Uses Manhattan Distance heuristic to find optimal paths
 */

import type { ControlPoint, NodeRect } from '../types';
import { areLinesReverseDirection, areLinesSameDirection } from '../geometry/edge';
import { isEqualPoint, isSegmentCrossingRect } from '../geometry/point';

interface GetAStarPathParams {
  /**
   * Collection of potential control points between sourceOffset and targetOffset
   */
  points: ControlPoint[];
  source: ControlPoint;
  target: ControlPoint;
...
/**
 * Utilizes the A* search algorithm combined with Manhattan Distance 
 * to find the optimal path for edges.
 */
export const getAStarPath = ({
  ...
  while (openSet.length) {
    ...
    const curGScore = gScore.get(current) ?? 0;
    ...
    for (const neighbor of neighbors) {
      ...
      const tentativeGScore = curGScore + estimateDistance(current, neighbor);
...
/**
 * Build the final path by backtracking through the cameFrom map
 */
const buildPath = (
